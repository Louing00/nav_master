...
- 支持用户注册，管理员可统一创建、删除、重置密码和分配管理员权限
- 搜索系统名称、描述与标签
- 系统详情页与功能索引
- 后台登录、应用管理、分类管理、站点设置
- 后台账号安全页支持当前用户修改密码
...
脚本会提示输入域名，并生成 Nginx 站点配置，将该域名反代到 `127.0.0.1:8081`。如果服务器已安装 `certbot`，脚本会尝试签发 HTTPS 证书。

也可以通过环境变量非交互执行：

```bash
sudo DOMAIN=nav.example.com bash scripts/install-nginx.sh
```
